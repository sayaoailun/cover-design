...
def _create_studio_shadow(self, cover_width: int, shadow_height: int, shadow_config: dict) -> Image.Image:
    ...
    """
    # ==================== 第1步：解析配置 ====================
    blur = shadow_config.get("blur", 6)                    # 高斯模糊半径
    opacity = shadow_config.get("opacity", 145)            # 阴影不透明度（0-255）
    fade_power = shadow_config.get("fade_power", 1.5)      # 渐变曲线指数
    shadow_color = tuple(shadow_config.get("color", [40, 40, 40]))  # 阴影颜色 RGB
    
    # ==================== 第2步：计算几何参数 ====================
    # 左右两边与底边夹角为 30 度
    angle_rad = math.radians(30)
    # 倾斜偏移 = shadow_height / tan(30°) 
    # tan(30°) ≈ 0.577，所以 offset ≈ shadow_height × 1.732
    ...
    # ==================== 第3步：创建透明度掩码 ====================
    # 创建灰度图（L 模式），所有像素初始值为 0（全透明）
    shadow_mask = Image.new("L", (canvas_width, canvas_height), 0)
    mask_pixels = shadow_mask.load()  # 获取像素访问对象，效率更高
    
    # ==================== 第4步：绘制平行四边形形状和透明度梯度 ====================
    for y in range(canvas_height):
        # 计算当前行在总高度中的进度（0 = 底部, 1 = 顶部）
        progress = y / canvas_height
        
        # 当前行的左右倾斜偏移
        # 底部（y=0）: offset = horizontal_offset（最宽）
        # 顶部（y=canvas_height）: offset = 0（最窄）
        left_offset = int(horizontal_offset * (1 - progress))
        right_offset = int(horizontal_offset * (1 - progress))
        
        # 计算当前行到底部的距离
        # 底部（y=canvas_height-1）: dist_from_bottom = 1.0（不透明）
        # 顶部（y=0）: dist_from_bottom = 0.0（全透明）
        dist_from_bottom = (canvas_height - 1 - y) / max(canvas_height - 1, 1)
        
        # 使用 fade_power 计算透明度值
        # fade_power 越大，上方淡化越快
        # 示例：fade_power=1.5，底部 255，顶部 0
        alpha_value = int(255 * (dist_from_bottom ** (1 / fade_power)))
        
        # 填充当前行的平行四边形区域
        # 从 left_offset 到 canvas_width - right_offset 的像素
        for x in range(left_offset, canvas_width - right_offset):
            mask_pixels[x, y] = alpha_value
    
    # ==================== 第5步：应用高斯模糊 ====================
    # 对透明度掩码进行高斯模糊，使阴影边界柔和
    shadow_mask = shadow_mask.filter(ImageFilter.GaussianBlur(blur))
    
    # ==================== 第6步：创建彩色阴影层 ====================
    # 创建 RGBA 图像，所有像素都是 shadow_color（默认 RGB(40,40,40)）
    color_layer = Image.new("RGBA", (canvas_width, canvas_height), shadow_color)
    
    # ==================== 第7步：应用透明度掩码 ====================
    # 使用模糊后的掩码作为 Alpha 通道
    # Image.eval() 对每个像素应用 lambda 函数
    # 将掩码像素值（0-255）转换为最终透明度：p * opacity / 255
    color_layer.putalpha(
        Image.eval(shadow_mask, lambda p: int(p * opacity / 255))
    )
