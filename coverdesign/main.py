...
import math
import pandas as pd
from PIL import Image, ImageEnhance, ImageDraw, ImageFont, ImageTk, ImageFilter, ImageChops
...
class ImageComposer:
    ...
    # Studio Shadow 生成
    # ===============================
    def _create_studio_shadow(self, cover_width: int, shadow_height: int, shadow_config: dict) -> Image.Image:
        """
        创建平行四边形阴影层
        
        Args:
            cover_width: Cover 的宽度
            shadow_height: 阴影的高度（length）
            shadow_config: 包含 blur, opacity, fade_power, color 的配置字典
        
        Returns:
            RGBA 模式的阴影图像，可直接进行 alpha_composite 操作
        """
        blur = shadow_config.get("blur", 6)
        opacity = shadow_config.get("opacity", 0.1)
        fade_power = shadow_config.get("fade_power", 1.5)
        shadow_color = tuple(shadow_config.get("color", [40, 40, 40]))
        
        # 计算平行四边形的倾斜参数
        # 左右两边与底边夹角为 30 度
        angle_rad = math.radians(30)
        horizontal_offset = int(shadow_height / math.tan(angle_rad))  # = shadow_height * sqrt(3)
        
        # 创建阴影画布，宽度包括倾斜偏移，高度是阴影高度
        canvas_width = cover_width + 2 * horizontal_offset
        canvas_height = shadow_height
        
        # 创建灰度图作为透明度掩码
        shadow_mask = Image.new("L", (canvas_width, canvas_height), 0)
        mask_pixels = shadow_mask.load()
        
        # 绘制平行四边形内部的透明度，从底部向上方渐变
        for y in range(canvas_height):
            # 计算当前行的左右边界（平行四边形的左右边界）
            # 在高度 y 处，边界随 y 的增加而缩小
            progress = y / canvas_height  # 0 at bottom, 1 at top
            
            # 计算当前行的左右偏移
            left_offset = int(horizontal_offset * (1 - progress))
            right_offset = int(horizontal_offset * (1 - progress))
            
            # 透明度从底部（y=canvas_height-1）向上方减弱
            # 距离底部的相对距离
            dist_from_bottom = (canvas_height - 1 - y) / max(canvas_height - 1, 1)
            max_alpha = shadow_config.get("max_alpha", 140)

            alpha_value = int(
                max_alpha *
                (dist_from_bottom ** fade_power)
)
            
            # 填充当前行的平行四边形区域
            for x in range(left_offset, canvas_width - right_offset):
                mask_pixels[x, y] = alpha_value
        
        # 应用模糊
        shadow_mask = shadow_mask.filter(ImageFilter.GaussianBlur(blur))
        
        # 创建彩色阴影层
        color_layer = Image.new("RGBA", (canvas_width, canvas_height), shadow_color)
        
        # 应用透明度掩码和阴影不透明度
        strength = shadow_config.get("strength", 0.7)

        color_layer.putalpha(
            Image.eval(
                shadow_mask,
                lambda p: int(
                    p * opacity * strength / 255
                )
            )
        )
    ...
    def _generate_single(self, row: dict, output_quality: int) -> tuple[str, str, Optional[Image.Image]]:
        ...
        try:
            ...
            # Studio Shadow - 生成阴影
            # ===========================
            if self.shadow_config.get("enabled", True):
                # 计算阴影高度
                shadow_height = int(cover.height * self.shadow_config.get("length_ratio", 0.34))
                
                # 创建阴影层
                shadow = self._create_studio_shadow(
                ...
                # 计算阴影位置
                # 阴影的底部与 cover 的底部对齐
                # 水平偏移是为了让平行四边形与 cover 底边中心对齐
                angle_rad = math.radians(20)
                horizontal_offset = int(shadow_height / math.tan(angle_rad))
                
                shadow_x = cover_x - horizontal_offset
                shadow_y = cover_bottom_y - shadow_height
            ...
            output_filename = f"{isbn}.{extension}"
            output_path = self.output_dir / output_filename

            # JPEG 不支持 Alpha
            if self.output_format.upper() in ("JPG", "JPEG"):
            ...
            result.save(
                output_path,
                format=self.output_format.upper() if self.output_format.upper() in ("JPG", "JPEG") else self.output_format,
                quality=output_quality,
                optimize=True
            )
            return isbn, "Success", result
        except Exception as e:
            return isbn, f"Error: {str(e)}", None
    ...
    def generate_all(
        ...
        for idx, (_, row) in enumerate(df.iterrows()):
            ...
            if on_update:
                on_update(idx + 1, total, success_count, isbn, status, preview)
...
class CoverGeneratorApp(tk.Tk):
    ...
    def setup_ui(self) -> None:
        ...
        center_frame.grid(row=0, column=1, sticky="nsew", padx=12)

        right_frame = ttk.Frame(main_frame)
        right_frame.grid(row=0, column=2, sticky="nsew", padx=(12, 0))

        ttk.Label(left_frame, text="设置", font=(None, 12, "bold")).grid(row=0, column=0, columnspan=2, sticky="w", pady=(0, 8))

        ttk.Label(left_frame, text="Excel:").grid(row=1, column=0, sticky="w")
        ...
        self.excel_entry.grid(row=1, column=0, sticky="ew", columnspan=2)
        ttk.Button(left_frame, text="浏览", command=self._browse_excel).grid(row=1, column=1, padx=4)

        ttk.Label(left_frame, text="Cover 目录:").grid(row=2, column=0, sticky="w")
        ...
        self.cover_entry.grid(row=2, column=0, sticky="ew", columnspan=2)
        ttk.Button(left_frame, text="浏览", command=self._browse_cover_dir).grid(row=2, column=1, padx=4)

        ttk.Label(left_frame, text="素材目录:").grid(row=3, column=0, sticky="w")
