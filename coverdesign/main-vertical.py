...
class ImageComposer:
    ...
    def _get_shadow_params(self, cover_height: int) -> dict:
        """
        根据 Cover 高度自动计算阴影参数
        """
        cfg = self.shadow_config
        base_height = cfg["base_height"]
        scale = cover_height / base_height
        ...
        return {
            ...
            "blur_radius": max(
                cfg["min_blur"],
                int(cfg["blur_radius"] * scale),
            ),
            "opacity": int(cfg["opacity"]),
    ...
    height_ratio: float,
        ...
        # 从配置中获取额外阴影参数
        cfg = self.shadow_config
        shadow_type = cfg.get("type", "studio")
        angle = cfg.get("angle", 30)
        length_ratio = cfg.get("length_ratio", 0.34)
        blur = cfg.get("blur", 6)
        fade_power = cfg.get("fade_power", 1.5)
        color = cfg.get("color", [40, 40, 40])
        enabled = cfg.get("enabled", True)
        # 从配置中获取透明度，覆盖函数参数中的opacity
        config_opacity = cfg.get("opacity", opacity)
        ...
        if shadow_type == "drop":
            ...
            # 长阴影：增加height_ratio
            height_ratio = min(height_ratio * 2, 0.8)
        ...
        # 如果阴影被禁用，返回空图像
        if not enabled:
            return Image.new("RGBA", cover.size, (0, 0, 0, 0))
        ...
        black = Image.new(
            "RGBA",
            (w, h),
            (color[0], color[1], color[2], config_opacity),
        )
        ...
        fade_start = int(h * (1 - height_ratio)) + margin

        for y in range(fade_start, canvas_h):

            factor = 1 - (
                (y - fade_start)
                / max(1, canvas_h - fade_start)
            )
    ...
    def _generate_single(self, row: dict, output_quality: int) -> tuple[str, str, Optional[Image.Image]]:
        ...
        try:
            ...
            params = self._get_shadow_params(
                cover.height
            )
            ...
            margin = params["blur_radius"] * 4

            shadow_x = cover_x - margin

            shadow_y = cover_y - margin
...
class CoverGeneratorApp(tk.Tk):
    def __init__(self, base_dir: Path):
        ...
        self.base_dir = base_dir
        self.config_path = base_dir / "config-vertical.json"
